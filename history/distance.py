...
CLOSE_THRESHOLD = 0.15  # Threshold for "too close" (normalized distance)
FAR_THRESHOLD = 0.05    # Threshold for "too far" (normalized distance)
...
def update_thresholds():
    global LOOK_THRESHOLD, SIDE_LOOK_THRESHOLD, CLOSE_THRESHOLD, FAR_THRESHOLD, JUST_RIGHT_THRESHOLD
    try:
        ...
        FAR_THRESHOLD = float(far_threshold_var.get())
...
far_threshold_var = tk.StringVar(value=str(FAR_THRESHOLD))
...
tk.Label(root, text="Far Threshold:").grid(row=3, column=0, padx=10, pady=5)
tk.Entry(root, textvariable=far_threshold_var).grid(row=3, column=1, padx=10, pady=5)

tk.Button(root, text="Update Thresholds", command=update_thresholds).grid(row=4, column=0, columnspan=2, pady=10)

status_label = tk.Label(root, text="Monitoring: OFF", fg="red")
status_label.grid(row=5, column=0, columnspan=2, pady=10)

tk.Button(root, text="Start/Stop Monitoring", command=toggle_monitoring).grid(row=6, column=0, columnspan=2, pady=10)

strict_mode_label = tk.Label(root, text="Strict Mode: OFF", fg="red")
strict_mode_label.grid(row=7, column=0, columnspan=2, pady=5)

tk.Button(root, text="Toggle Strict Mode", command=toggle_strict_mode).grid(row=8, column=0, columnspan=2, pady=10)

# Add a new label for the distance status
distance_status_label = tk.Label(root, text="Distance Status: Normal", fg="green")
distance_status_label.grid(row=9, column=0, columnspan=2, pady=5)
